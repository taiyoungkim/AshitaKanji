...
const DEFAULT_KANJI_MEANING_KO_BACKFILL = resolve(
  ROOT,
  'data/track-a/kanji_meaning_ko_backfill.json',
);
const DEFAULT_KANJI_RADICAL_NAMES_KO = resolve(
  ROOT,
  'data/track-a/kanji_radical_names_ko.json',
);
...
const WORD_CURATION_VERSION = '13';
...
// is produced at this schema (all v1–v7 tables/columns/indexes), so app_meta
// schema_version is stamped here too — otherwise launch migrations re-run ALTERs
// against columns that already exist (e.g. "duplicate column name: frequency").
// When bumping, also add the new columns to the word CREATE below so the stamp is truthful.
const SCHEMA_VERSION = 7;
// 표기 변형(카나/한자) 통합으로 9개 표제어를 접었다 —
// data/pdf-vocab/orthography_variant_review_queue.csv 참고.
const TARGET_COUNTS = { N5: 398, N4: 742, N3: 1488, N2: 1906, N1: 2484 };
// 회독(read-through) frozen chapter assignments — generated by gen-reading-chapters.py
const READING_CHAPTERS_PATH = resolve(ROOT, 'data/track-a/reading_chapters.json');
const READING_CHAPTERS = existsSync(READING_CHAPTERS_PATH)
  ? JSON.parse(readFileSync(READING_CHAPTERS_PATH, 'utf8'))
  : {};
...
const RESPONSE_EXPRESSION_SURFACES = new Set([
  'あ',
  'ああ',
  'あの',
  'いいえ',
...
const DEMONSTRATIVE_MANNER_SURFACES = new Set([
  'あんな',
  'こんな',
  'そんな',
  'どんな',
...
const ALLOWED_PURE_NUMERALS = new Set(['一', '十']);
const PURE_NUMERAL_READINGS = new Set([
  'ゼロ',
  'れい',
  'いち',
  'に',
...
const PURE_NUMERAL_SURFACES = new Set([
  '〇',
  '零',
  '一',
  '二',
...
const kanjiRadicalNamesKoPath = resolve(
  args.kanjiRadicalNamesKo ?? DEFAULT_KANJI_RADICAL_NAMES_KO,
);
const kanjiRadicalNamesKo = readKanjiRadicalNamesKo(kanjiRadicalNamesKoPath);
...
function readKanjiMeaningKoBackfill(path) {
  if (!existsSync(path)) return new Map();
  const raw = JSON.parse(readFileSync(path, 'utf8'));
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    throw new Error(`Kanji Korean meaning backfill must be an object: ${rel(path)}`);
  }

  const out = new Map();
  for (const [literal, values] of Object.entries(raw)) {
    const normalized = parseArrayish(values).map(clean).filter(Boolean);
    if (!literal || normalized.length === 0) {
      throw new Error(`Invalid kanji Korean meaning backfill entry: ${literal || '<empty>'}`);
    }
    out.set(literal, unique(normalized));
  }
  return out;
}

function readKanjiRadicalNamesKo(path) {
  if (!existsSync(path)) throw new Error(`Kanji radical Korean names file missing: ${rel(path)}`);
  const raw = JSON.parse(readFileSync(path, 'utf8'));
  if (!raw || !Array.isArray(raw.radicals) || raw.radicals.length !== KANJI_RADICALS.length) {
    throw new Error(`Kanji radical Korean names must contain all 214 radicals: ${rel(path)}`);
  }

  const out = new Map();
  for (const item of raw.radicals) {
    const number = Number(item?.number);
    const radical = clean(item?.radical);
    const nameKo = clean(item?.name_ko);
    const expectedRadical = radicalByNumber(number);
    if (!Number.isInteger(number) || number < 1 || number > KANJI_RADICALS.length) {
      throw new Error(`Invalid radical number in ${rel(path)}: ${item?.number}`);
    }
    if (!expectedRadical || radical !== expectedRadical || !nameKo || out.has(number)) {
      throw new Error(`Invalid radical Korean name entry in ${rel(path)}: ${JSON.stringify(item)}`);
    }
    out.set(number, { radical, nameKo });
  }
  return out;
}
...
function buildKanjiQaRows(wordRows, kanjidic) {
  ...
  for (const literal of needed) {
    ...
    const currentQaNote = clean(current?.qa_note);
    const qaNote = !currentQaNote || currentQaNote.startsWith('Auto draft:')
      ? AUTO_KANJI_QA_NOTE
      : currentQaNote;
    ...
    out.push({
      ...
      radical_name_ko:
        clean(current?.radical_name_ko) ||
        kanjiRadicalNamesKo.get(entry.radicalNumber)?.nameKo ||
        '',
...
function deriveKanjiMeaningKo(literal, wordRows) {
  ...
  const backfilled = kanjiMeaningKoBackfill.get(literal);
...
function selectRows(sourceRows) {
  ...
  for (const row of sourceRows) {
    ...
    // Dedup globally on surface+reading (level excluded) so the same word appearing
    // in two levels collapses to one row — required because word.id no longer encodes
    // level. `seen` spans all levels, so first occurrence wins.
    const key = duplicateWordKey({ surface, reading_kana: reading });
    if (seen.has(key)) continue;
    seen.add(key);
    buckets[level].push({
      ...makeQaRow({
        id: makeId(surface, reading),
        level,
        surface,
        reading,
        english,
  ...
  for (const source of sourceRows) {
    ...
    out.push(normalizeQaRows([
      makeQaRow({
        ...
        qaNote: 'Backfilled to maintain active vocabulary target; Korean meaning draft required',
      }),
    ])[0]);
    deficits[level] -= 1;
...
function buildDatabase(rows, outPath, kanjiQaRows, naverExampleRows = []) {
  ...
  execSql(tmp, [
    ...
    // v3 회독 index — frequency/reading_chapter columns already in word table above.
    'CREATE INDEX idx_word_reading_chapter ON word(level, reading_chapter);',
    `CREATE TABLE kanji (
    ...
    `CREATE TABLE review_log (
      ...
      reveal_ms INTEGER,
      session_id INTEGER,
      scheduling_applied INTEGER NOT NULL DEFAULT 1 CHECK (scheduling_applied IN (0,1)),
      FOREIGN KEY (word_id) REFERENCES word(id),
    ...
    'CREATE INDEX idx_session_source ON session(source_session_id);',
    `CREATE TABLE events (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ts INTEGER NOT NULL,
      type TEXT NOT NULL,
    ...
    // v4 누적 회독 — per-(word_id, chapter) known state. Ships empty (user data),
    // but the table must exist so seed is honestly at schema v4.
    ...
    'CREATE INDEX idx_reading_progress_chapter ON reading_progress(chapter, known);',
...
function inspectDatabase(dbPath) {
  ...
  return {
    ...
    kanji: {
      ...
      qa: kanjiQa,
      missingMeaningsKo: tableCount(
        'kanji',
        `meanings_ko IS NULL OR trim(meanings_ko)='' OR meanings_ko='[]'`,
      ),
      missingRadicalNamesKo: tableCount(
        'kanji',
...
function vocabularyExclusionReason(row) {
  ...
  if (/^[×○◯△□☆★◎※]+$/.test(surface)) {
    return '기호만으로 된 표제어라 단어 학습 항목에서 제외';
  }
  if (RESPONSE_EXPRESSION_SURFACES.has(surface)) {
    return '감탄사·응답 표현이라 단어 학습 표제어에서 제외';
  }
  if (DEMONSTRATIVE_MANNER_SURFACES.has(surface)) {
    return '지시 관형·방식 표현이라 단어 학습 표제어에서 제외';
  }
  if (PURE_NUMERAL_SURFACES.has(surface) && !ALLOWED_PURE_NUMERALS.has(surface) && hasPureNumeralReading(reading)) {
    return '순수 숫자 항목은 一/十만 유지하고 제외';
  }
  if (/^[一二三四五六七八九十百千万億兆零〇]+(?:日|つ)$/.test(surface)) {
    return '날짜·개수 세기 표현이라 독립 단어 학습 표제어에서 제외';
  }
  return '';
}

function hasPureNumeralReading(reading) {
  return clean(reading)
    .split(/[;；,、\s]+/)
    .filter(Boolean)
    .some((part) => PURE_NUMERAL_READINGS.has(part));
}
...
// Stable identity: derived only from canonicalized surface + reading (+ optional
// sense key for homographs). Deliberately EXCLUDES level and any positional index,
// so JLPT level corrections (N4→N3) or dedup/backfill reordering never change a
// word's id — the same word keeps the same id across rebuilds, preserving user_card.
// senseKey is reserved for the rare same-surface+same-reading-different-meaning case;
// leave empty for normal words so single-sense ids stay stable if it's added later.
function makeId(surface, reading, senseKey = '') {
  const norm = (s) => clean(s).normalize('NFKC');
  const SEP = '\u0001'; // separates surface/reading/sense in the hash basis
...
// Fail the build loudly on duplicate word ids. With hash-based ids a collision means
// two rows share surface+reading (a real homograph needing a distinct senseKey, or a
// dedup gap) — silently importing would violate the word.id PRIMARY KEY or drop a row.
function assertUniqueWordIds(rows) {
  const byId = new Map();
  const collisions = [];
  for (const row of rows) {
    const prev = byId.get(row.id);
    if (prev) {
      collisions.push(`${row.id}: "${prev.surface}/${prev.reading_kana}" vs "${row.surface}/${row.reading_kana}"`);
    } else {
      byId.set(row.id, row);
    }
  }
  if (collisions.length > 0) {
    throw new Error(
      `Duplicate word ids (${collisions.length}) — assign a distinct senseKey or fix dedup:\n` +
...
Options:
  ...
  --kanjiRadicalNamesKo PATH
                   Korean names for all 214 Kangxi radicals (default: ${rel(DEFAULT_KANJI_RADICAL_NAMES_KO)})
...
function printReport(report) {
  ...
  console.log(`kanji: total=${report.kanji.total}, missing_meanings_ko=${report.kanji.missingMeaningsKo}, missing_radical_names_ko=${report.kanji.missingRadicalNamesKo}, word_links=${report.kanji.wordLinks}, words=${report.kanji.kanjiBearingWords}`);
