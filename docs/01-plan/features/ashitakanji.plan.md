...
> 작성일: 2026-05-29 (**v0.7.4 - 개발 워크플로 결정 추가**)
...
>      `deploy-pages.sh` 스크립트, grep gate 자기참조 해소, 404 해결 절차 명문화
> **v0.7.4**: §29 Development Workflow 추가 — Mac local + Expo Dev Client,
>      bkit PDCA 단일 세션, Track A+B 병행, main+feature/* + PDCA phase commit,
>      Expo Go→Dev Client→EAS 파이프라인 명시
...
> 다음 단계(Design/Do/Analysis)로 전파되는 핵심 컨텍스트. 세션 간 컨텍스트 단절 방지.

| 축 | 내용 |
|---|---|
| **WHY** | 한자/가나/뜻 동시 노출 = 수동 읽기 + 망각. reveal 강제 + FSRS 간격반복 + 도파민 루프 = 매일 끝낼 수 있는 평생 단어장. 레벨업해도 같은 앱 |
| **WHO** | 1차: 장기 학습자 (수년에 걸쳐 N5→N1, 하루 15-30분). 2차: 시험 임박 헤비유저 (빠른 훑기/약점 복습 주 사용). 단일 기기 / 다기기 동기화는 V1.2+ |
| **RISK** | 1) 6,200 검수 부담 (풀타임 8-10일, 게이트 ≤1%) 2) Tatoeba attribution 미준수 → 라이선스 분쟁 3) "6,200 = JLPT 전체" 카피 오해 → 별점 4) Placeholder 회귀 5) 데이터 소실 (앱 삭제 시) 6) GPT 초안 anchoring bias |
| **SUCCESS** | 출시: Core 6,200 verified 100% / 5종 카드 타입 동작 / 일일형 FSRS+Done 화면 / 레벨별 진척 5개+전체 1개 / JSON export / 사용자 학습데이터 외부 송신 0건 / OTA off / Privacy+Support URL HTTP 200 / Placeholder 0건. 운영: 본인 streak 5일+, 세션 완료율 70%+, Mature 50%+ |
| **SCOPE** | **MVP**: 아시타칸지 = N5-N1 6,200 verified + 레벨 선택/혼합 + reveal UX + FSRS 4단계 + 일일 신규 12개 (5-50) + 빠른 훑기 50/100/200/300 + 약점 복습 + Again 다음 due 이월 + Done 화면 + 통계(레벨별+전체) + TTS (ja-JP) + JSON export. **V1.1**: 알림, 한자 분해, 연상 노트, Leech 격리, 잔디 그래프, 단어 검색, 카드 오류 신고, opt-in 분석. **V1.2+**: 네이버 사전, GPT 질문(구독+서버 프록시), 인터리빙, 다크모드 토글, 위젯, IAP 광고 제거, 클라우드 동기화 |

### 핵심 결정 체인 (Plan→Design→Do 전파용)
- **데이터**: Kaggle Robin Pourtaud `jlpt-words-by-level` (CC BY 4.0) 단일 출처 — N1 포함 확인 완료
- **검수**: GPT-4o 초안 + 100% 사람 검수 — AI = 도구, 콘텐츠 책임 = 사람 (Google Play AI policy 준수)
- **알고리즘**: ts-fsrs npm (FSRS) — SM-2 대비 30% 적은 복습량
- **명명**: 아시타칸지 (明日漢字) — "빈도 상위" 표현 금지, "편집자 큐레이션"으로 통일
- **통신**: MVP OTA off (`expo.updates.enabled=false`) — 학습 데이터 + 시스템 통신 0건
- **출시 게이트 5**: 검수 오답률 / Placeholder 회귀 / URL HTTP 200 / mitmproxy 0건 / OTA off
...
CREATE TABLE word (
  ...
  furigana TEXT,                   -- 후리가나 (B형 한자 활용형 용)
...
### "오늘 완료" 정의
세션은 due 복습을 먼저 처리하고, backlog 정책으로 허용된 신규를 이어서 처리한다.
각 카드는 한 번만 노출되며 큐를 비우면 **"오늘 끝!" 화면**으로 이동한다.
Again은 같은 세션에 다시 넣지 않고 일일형 FSRS가 계산한 다음 due로 이월한다.
...
| 버튼 | 의미 | 효과 |
|---|---|---|
| Again | 못 떠올림 | 같은 세션에 재출제하지 않고 다음 FSRS due로 이월 |
| Hard | 떠올렸지만 힘듦 | 짧은 간격 |
| Good | 정상 회상 | 표준 간격 |
| Easy | 즉답 | 긴 간격 ×1.3 |
...
### 큐 우선순위
1. 기존 due 복습 전체 (선택 레벨과 무관, 오래 밀린 순)
2. 선택 레벨의 신규 카드 (backlog가 크면 자동 감축)

레벨 선택은 신규 카드 범위만 바꾼다. 이미 학습을 시작한 카드의 due 복습은 선택을 해제한 레벨도 계속 출제해 FSRS 일정을 보존한다.
...
아직 due가 아닌 기존 카드는 정규 FSRS 일정을 유지하고 보강 복습 로그만 기록한다. 이미 due인 카드와 빠른 훑기에서 새로 편입하는 카드에만 FSRS 재스케줄을 적용한다.
...
| B3. FSRS + 큐 + Again 정책 | ts-fsrs 통합, 복습 우선 일일 큐, review_log | 1.5일 |
...
- [x] 복습 우선 일일 큐 + Again의 다음 due 이월 동작
- [x] "Done!" 화면 도달 가능 (정의 = 계획된 일일 큐를 비움)
...
- ✅ Again 정책 = 같은 세션 재출제 없이 다음 FSRS due로 이월
...
## 29. Development Workflow (v0.7.3, 개발 단계 결정)

### 환경
- **Mac local + Expo Dev Client**
- iOS 시뮬레이터 (Xcode) + Android 에뮬레이터 (Android Studio) + 본인 실기기 1대 (개발 메인)
- 출시 직전 Device QA Matrix (PLAN §20) P0 4종 통과

### 협업 방식
- **bkit PDCA 단일 세션 흐름** (Agent Teams 미사용)
- 진행: `/pdca design ashitakanji` → `/pdca do ashitakanji` → `/pdca analyze ashitakanji` → `/pdca iterate ashitakanji` → `/pdca report ashitakanji`
- 환경 변수: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 설정 X
- Skill 진입 시점: Plan→Design 전환, Design→Do 전환, Do→Check 전환마다

### 작업 순서 (Track A + B 병행)
```
Day 1     Day 2-3    Day 4-5    Day 6-10    Day 11-14    Day 15-17
─────     ──────     ──────     ─────────   ──────────   ─────────
A1: DL    A2: 정규화  A3: GPT    A4: 검수     A4: 검수      A5: 예문
                     초안 배치   400-500/d    (병목)        A6: build-db
                                                                  ↓
B1: Expo  B2: 카드    B3: FSRS   B4: 고강도    B5: Card UX   B7: JSON export
   골격    타입       + Again     모드          + Done        B8: 디자인 패스
                                                            
                                                            Track C: 출시 준비
                                                            C1-C4 (2일)
```
- Track A 끝나는 시점에 Track B는 B6/B7 완료 → C1 시작 동기화
- B는 데이터 없어도 더미 1-2개로 UX 개발 가능 (A2 끝나면 실 데이터로 교체)

### Git 전략
- 브랜치: `main` + `feature/*`
- feature 브랜치 명명:
  - `feature/data-pipeline` (Track A)
  - `feature/app-skeleton`
  - `feature/card-types`
  - `feature/fsrs-queue`
  - `feature/scan-mode` (빠른 훑기)
  - `feature/done-screen`
  - `feature/json-export`
  - `feature/release-prep`
- **커밋 단위 = PDCA phase 경계**:
  - Design 문서 완성 시 1 커밋
  - Do (구현) 끝 시 1 커밋 (또는 화면/모듈 단위)
  - Check (gap analysis) 결과 1 커밋
  - Iterate 완료 시 1 커밋
- 브랜치 → main 머지: 본인 머지 OK (PR 절차 생략)
- 커밋 메시지 컨벤션: `feat(card): reveal toggle`, `chore(data): normalize N5`, `docs(plan): v0.7.3`

### 빌드 파이프라인
| 단계 | 도구 | 시점 |
|---|---|---|
| 개발 | Expo Go → Expo Dev Client (sqlite 같은 네이티브 모듈 들어가는 시점부터 Dev Client) | Day 1+ |
| 내부 테스트 | EAS Build + TestFlight (iOS) + Play Internal Testing (Android) | Day 11+ |
| 출시 | EAS Production → App Store / Google Play | Day 15+ |
...
# iOS: Xcode 설치 (App Store)
# Android: Android Studio 설치 + emulator 생성

# Expo 계정 + EAS 로그인
eas login

# 프로젝트 init
pnpm create expo-app ashitakanji --template
cd ashitakanji
git init && git remote add origin git@github.com:taiyoungkim/AshitaKanji.git
```

### 출시 게이트 자동화 (선택, V1.1)
- GitHub Actions 워크플로 `.github/workflows/release-gate.yml`:
  - placeholder 회귀 스캔
  - URL HTTP 200 체크
  - DB 크기 < 20MB
  - OTA off 확인 (app.json 검증)
- MVP는 수동 (`make release-check` 같은 로컬 스크립트 OK)
