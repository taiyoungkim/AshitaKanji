...
| **SUCCESS** | 출시: Core 6,200 verified 100% / 5종 카드 타입 동작 / FSRS+Again 미니라운드+Done 화면 / 레벨별 진척 5개+전체 1개 / JSON export / 사용자 학습데이터 외부 송신 0건 / OTA off / Privacy+Support URL HTTP 200 / Placeholder 0건. 운영: 본인 streak 5일+, 세션 완료율 70%+, Mature 50%+ |
| **SCOPE** | **MVP**: 아시타칸지 = N5-N1 6,200 verified + 레벨 선택/혼합 + reveal UX + FSRS 4단계 + 일일 신규 12개 (5-50) + 빠른 훑기 50/100/200/300 + 약점 복습 + Again 세션끝 미니라운드 + Done 화면 + 통계(레벨별+전체) + TTS (ja-JP) + JSON export. **V1.1**: 알림, 한자 분해, 연상 노트, Leech 격리, 잔디 그래프, 단어 검색, 카드 오류 신고, opt-in 분석. **V1.2+**: 네이버 사전, GPT 질문(구독+서버 프록시), 인터리빙, 다크모드 토글, 위젯, IAP 광고 제거, 클라우드 동기화 |
